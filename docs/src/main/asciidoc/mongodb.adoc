...
MongoDB allows you to configure multiple connection.
It works exactly the same way as a single connection, with one important change: a connection needs to have a name.
...
=== Named Mongo connection Injection

When using multiple connection, each `MongoClient` also has the `io.quarkus.mongodb.runtime.MongoClientConnection` qualifier with the name of the connection in the property as the value.
Using the above properties to configure three different connections, you can also inject each one as follows:
...
@Inject
@MongoClientConnection("users")
MongoClient mongoClient1;

@Inject
@MongoClientConnection("inventory")
